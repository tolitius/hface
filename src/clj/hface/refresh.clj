...
(defn add-map-ratings [p-stats    ;; previous stats
                       ...
                       interval]
  (let [m-stats #(-> % :aggregated :map-stats)
        previous (m-stats p-stats)
        current (m-stats c-stats)]
    (assoc-in c-stats [:aggregated :map-stats]
...
(defn refresh-stats 
  ...
  (let [current (cluster-stats)
        with-rates (add-map-ratings @previous current interval)]     ;; TODO: refactor out and add other ratings
